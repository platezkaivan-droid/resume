...
const translations = {
    ...
    en: {
        ...
        about_p2: 'Built 9+ projects: a Telegram bot deployed on Render with Docker + CI/CD, a React Native mobile app with real-time chat, my own terminal VPN client, a VPN-selling Telegram bot with real payments (AmneziaWG + CryptoBot), a Minecraft mod port (246 mixins), and 4 Fabric mods from scratch.',
        ...
        ach_bot: '🛒 <strong>VPN service in production</strong> — a Telegram bot selling VPN: AmneziaWG backend with per-user peers, CryptoBot payments, web dashboard and referrals — running for real users.',
        ach_5: '🔐 <strong>VPN client from scratch</strong> — terminal client in Python on top of the mihomo core: TUI with ping and traffic, one-command install, releases on GitHub.',
        ach_6: '📂 <strong>Open Source</strong> — all projects are public on GitHub with docs and releases.',
        edu_title: 'Education',
        ...
        proj_dvpn_title: 'dvpn — terminal VPN client',
        proj_dvpn_company: 'Personal project',
        proj_dvpn_1: '<strong>🖥️ TUI interface</strong> — pick a server from a subscription, sort by ping, live traffic and quota right in the console',
        proj_dvpn_2: '<strong>🔌 mihomo core</strong> — supports vless (Reality, xhttp, gRPC), hysteria2, trojan, vmess, shadowsocks',
        proj_dvpn_3: '<strong>⚡ One-command install</strong> — PowerShell / curl, no admin rights, portable mode',
        proj_dvpn_4: '<strong>🌐 Cross-platform</strong> — Windows and Linux, automatic core download, system proxy management',
        proj_dvpn_stack: '<strong>🛠️ Stack:</strong> Python, mihomo (Clash), TUI, GitHub Releases',
        proj_vamos_title: 'VAMOS VPN — Telegram VPN-selling bot with a web dashboard',
        proj_vamos_company: 'Personal project (<a href="https://t.me/haribo_vamos_vpn_bot" target="_blank">@haribo_vamos_vpn_bot</a>)',
        proj_vamos_1: '<strong>💳 Real payments</strong> — plans, CryptoBot checkout and internal balance, referrals, wheel of fortune, auto promos in the channel — the service runs on autopilot',
        proj_vamos_2: '<strong>🔐 AmneziaWG core</strong> — userspace amneziawg-go daemon, per-user peer key, reverse split tunnel "everything except Russia" (~4,600 prefixes, cron-regenerated)',
        proj_vamos_3: '<strong>🛡️ Censorship circumvention</strong> — TikTok traffic marked via nftables and routed into Cloudflare WARP; dnsmasq serves IPv4 only and resolves RU domains via Yandex DNS',
        proj_vamos_4: '<strong>🌐 Web dashboard</strong> (Telegram WebApp) — Cloudflare → nginx → Python, initData auth (HMAC-SHA256): plan purchases, history, one-tap connect',
        proj_vamos_5: '<strong>🗄️ Operations</strong> — SQLite + systemd, invoice polling every 20 s, all-service status with a single command',
        proj_vamos_stack: '<strong>🛠️ Stack:</strong> Python, aiogram 3, SQLite, AmneziaWG (WireGuard), nftables, dnsmasq, Cloudflare WARP, nginx, systemd, CryptoBot',
